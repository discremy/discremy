...
async function init(targetDir) {
    ...
    const target = typeof targetDir === 'string' ? targetDir.trim() : null;
    const isCurrentDir = target === '.';
    let projectName;
    let projectPath;
    let projectDirDisplay;
    ...
    if (isCurrentDir) {
        ...
        if (target) {
            projectPath = path.resolve(process.cwd(), target);
            projectName = path.basename(projectPath);
            projectDirDisplay = target;
        } else {
    ...
    if (!isCurrentDir) {
        console.log(`  ${paint('1.', 'dim')} cd ${projectDirDisplay}`);
